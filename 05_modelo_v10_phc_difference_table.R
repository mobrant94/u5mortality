...
fwrite_ptbr_safe <- function(dt, file) {
  # CSV para Excel em configuracao brasileira:
  # - separador de campos = ponto e virgula
  # - separador decimal = virgula
  # - BOM = TRUE para preservar acentos no Excel/Windows
  dt <- sanitize_for_export(dt)
  dir_create_safe(dirname(file))
  data.table::fwrite(
    dt,
...
fmt_num_pt <- function(x, digits = 2) {
  out <- rep("", length(x))
  ok <- !is.na(x) & is.finite(x)
  out[ok] <- formatC(
    x[ok],
...
fmt_pct_pt <- function(x, digits = 1) {
  out <- rep("", length(x))
  ok <- !is.na(x) & is.finite(x)
  out[ok] <- paste0(fmt_num_pt(x[ok], digits), "%")
  out
}
...
fmt_rcr_ic_pt <- function(est, lo, hi, digits = 2) {
  out <- rep("", length(est))
  ok <- !is.na(est) & is.finite(est) &
    !is.na(lo) & is.finite(lo) &
    !is.na(hi) & is.finite(hi)
  out[ok] <- paste0(
    fmt_num_pt(est[ok], digits),
    " (",
    fmt_num_pt(lo[ok], digits),
    "; ",
    fmt_num_pt(hi[ok], digits),
    ")"
  )
  out
}

fmt_pct_ic_pt <- function(est, lo, hi, digits = 1) {
  out <- rep("", length(est))
  ok <- !is.na(est) & is.finite(est) &
    !is.na(lo) & is.finite(lo) &
    !is.na(hi) & is.finite(hi)
  out[ok] <- paste0(
    fmt_pct_pt(est[ok], digits),
    " (",
    fmt_pct_pt(lo[ok], digits),
    "; ",
    fmt_pct_pt(hi[ok], digits),
    ")"
  )
  out
}
...
out_dir_v10 <- file.path(dir_v9, "v10_phc_en_2lines_same_y_diff_table")
...
# Recorte temporal solicitado: analise ate 2021.
base0 <- base0[!is.na(ano) & ano <= ano_final_analise]

if (nrow(base0) == 0) {
  stop("A base ficou vazia apos aplicar ano <= ", ano_final_analise, ".")
}
...
fwrite_safe(
  data.table::data.table(
    ...
    ano_final_analise = ano_final_analise,
...
# ============================================================
# 8B. TABELA PRINCIPAL: DIFERENCA ENTRE POLITICA ISOLADA E POLITICA + APS
# ============================================================

# Interpretacao da medida principal:
#   diferenca_percentual = (interaction ratio - 1) * 100
#
# Onde:
#   interaction ratio =
#     (efeito da politica quando APS aumentou) /
#     (efeito da politica quando APS nao aumentou)
#
# Portanto:
#   valor < 0: o efeito da politica foi mais favoravel quando combinado com APS
#   valor > 0: o efeito da politica foi mais favoravel sem aumento conjunto de APS
#
# Esta tabela deve ser a principal para comparar "politica sozinha"
# versus "politica + APS", pois preserva a estrutura de contraste do modelo.

criar_tabela_diferenca_policy_phc <- function(res) {
  res <- data.table::as.data.table(data.table::copy(res))

  tipos_necessarios <- c(
    "policy_effect_without_phc_stagnation",
    "policy_effect_with_phc_stagnation",
    "phc_modification_stagnation"
  )

  res <- res[tipo_contraste %in% tipos_necessarios]

  if (nrow(res) == 0) {
    return(list(raw = data.table::data.table(), formatted = data.table::data.table()))
  }
  ...
  base_cols <- intersect(base_cols, names(res))
  r <- res[, ..base_cols]

  sem_aps <- r[tipo_contraste == "policy_effect_without_phc_stagnation"]
  com_aps <- r[tipo_contraste == "policy_effect_with_phc_stagnation"]
  dif     <- r[tipo_contraste == "phc_modification_stagnation"]
  ...
  sem_aps <- sem_aps[, .(
    ...
    marcador_label_longo,
    classe,
    rcr_politica_sozinha = medida,
    ic95_inf_politica_sozinha = ic95_inf,
    ic95_sup_politica_sozinha = ic95_sup,
    p_politica_sozinha = p_valor,
    n_linhas_politica_sozinha = n_linhas,
    n_municipios_politica_sozinha = n_municipios,
    n_anos_politica_sozinha = n_anos,
    erro_politica_sozinha = erro_contraste
  )]
  ...
  com_aps <- com_aps[, .(
    ...
    marcador_label_longo,
    classe,
    rcr_politica_com_aps = medida,
    ic95_inf_politica_com_aps = ic95_inf,
    ic95_sup_politica_com_aps = ic95_sup,
    p_politica_com_aps = p_valor,
    n_linhas_politica_com_aps = n_linhas,
    n_municipios_politica_com_aps = n_municipios,
    n_anos_politica_com_aps = n_anos,
    erro_politica_com_aps = erro_contraste
  )]
  ...
  dif <- dif[, .(
    ...
    marcador_label_longo,
    classe,
    razao_interacao_aps = medida,
    ic95_inf_razao_interacao_aps = ic95_inf,
    ic95_sup_razao_interacao_aps = ic95_sup,
    diferenca_percentual_com_aps_vs_sozinha = variacao_relativa_pct,
    ic95_inf_diferenca_percentual = (ic95_inf - 1) * 100,
    ic95_sup_diferenca_percentual = (ic95_sup - 1) * 100,
    p_diferenca = p_valor,
    n_linhas_modelo = n_linhas,
    n_municipios_modelo = n_municipios,
    n_anos_modelo = n_anos,
    ano_ref_estagnacao = ano_ref_estagnacao,
    erro_diferenca = erro_contraste
  )]
  ...
  out[, faixa_label_en := age_label_en(faixa_analise)]

  out[, interpretacao_diferenca := data.table::fcase(
    !is.na(diferenca_percentual_com_aps_vs_sozinha) &
      diferenca_percentual_com_aps_vs_sozinha < 0 &
      !is.na(p_diferenca) & p_diferenca < 0.05,
    "Favorece politica + APS; diferenca estatisticamente significativa",
    !is.na(diferenca_percentual_com_aps_vs_sozinha) &
      diferenca_percentual_com_aps_vs_sozinha < 0,
    "Favorece politica + APS; diferenca nao significativa",
    !is.na(diferenca_percentual_com_aps_vs_sozinha) &
      diferenca_percentual_com_aps_vs_sozinha > 0 &
      !is.na(p_diferenca) & p_diferenca < 0.05,
    "Favorece politica sozinha; diferenca estatisticamente significativa",
    !is.na(diferenca_percentual_com_aps_vs_sozinha) &
      diferenca_percentual_com_aps_vs_sozinha > 0,
    "Favorece politica sozinha; diferenca nao significativa",
    default = "Inconclusivo"
  )]

  out[, nota_metodologica := paste0(
    "Diferenca percentual = (razao de interacao - 1) * 100. ",
    "A razao de interacao compara o efeito da politica com APS versus ",
    "o efeito da politica sem aumento conjunto de APS, no periodo de estagnacao."
  )]
  ...
  data.table::setcolorder(
    ...
    c(
      ...
      "marcador_label",
      "marcador_label_longo",
      "rcr_politica_sozinha",
      "ic95_inf_politica_sozinha",
      "ic95_sup_politica_sozinha",
      "p_politica_sozinha",
      "rcr_politica_com_aps",
      "ic95_inf_politica_com_aps",
      "ic95_sup_politica_com_aps",
      "p_politica_com_aps",
      "razao_interacao_aps",
      "ic95_inf_razao_interacao_aps",
      "ic95_sup_razao_interacao_aps",
      "diferenca_percentual_com_aps_vs_sozinha",
      "ic95_inf_diferenca_percentual",
      "ic95_sup_diferenca_percentual",
      "p_diferenca",
      "interpretacao_diferenca",
      "n_linhas_modelo",
      "n_municipios_modelo",
      "n_anos_modelo",
      "ano_ref_estagnacao",
  ...
  out_fmt <- data.table::copy(out)

  out_fmt[, `:=`(
    efeito_politica_sozinha_rcr_ic95 = fmt_rcr_ic_pt(
      rcr_politica_sozinha,
      ic95_inf_politica_sozinha,
      ic95_sup_politica_sozinha,
      digits = 2
    ),
    p_politica_sozinha_formatado = fmt_p_pt(p_politica_sozinha),
    efeito_politica_com_aps_rcr_ic95 = fmt_rcr_ic_pt(
      rcr_politica_com_aps,
      ic95_inf_politica_com_aps,
      ic95_sup_politica_com_aps,
      digits = 2
    ),
    p_politica_com_aps_formatado = fmt_p_pt(p_politica_com_aps),
    razao_interacao_aps_ic95 = fmt_rcr_ic_pt(
      razao_interacao_aps,
      ic95_inf_razao_interacao_aps,
      ic95_sup_razao_interacao_aps,
      digits = 2
    ),
    diferenca_percentual_ic95 = fmt_pct_ic_pt(
      diferenca_percentual_com_aps_vs_sozinha,
      ic95_inf_diferenca_percentual,
      ic95_sup_diferenca_percentual,
      digits = 1
    ),
    p_diferenca_formatado = fmt_p_pt(p_diferenca)
  )]

  out_fmt <- out_fmt[, .(
    faixa_etaria = faixa_analise,
    faixa_etaria_en = faixa_label_en,
    classe,
    politica = marcador_label,
    politica_longa = marcador_label_longo,
    efeito_politica_sozinha_rcr_ic95,
    p_politica_sozinha = p_politica_sozinha_formatado,
    efeito_politica_com_aps_rcr_ic95,
    p_politica_com_aps = p_politica_com_aps_formatado,
    razao_interacao_aps_ic95,
    diferenca_percentual_com_aps_vs_sozinha_ic95 = diferenca_percentual_ic95,
    p_diferenca = p_diferenca_formatado,
    interpretacao_diferenca,
    n_linhas_modelo,
    n_municipios_modelo,
    n_anos_modelo,
  ...
  # Ordenacao para leitura: geral primeiro, depois faixas; dentro de cada faixa,
  # saneamento, vacinas e protecao social mantidos pelo catalogo original.
  out[, ordem_faixa := match(faixa_analise, faixa_levels)]
  out[is.na(ordem_faixa), ordem_faixa := 999L]
  out[, ordem_politica := match(marcador, catalogo$marcador)]
  out[is.na(ordem_politica), ordem_politica := 999L]
  data.table::setorder(out, ordem_faixa, ordem_politica)

  out_fmt[, ordem_faixa := match(faixa_etaria, faixa_levels)]
  out_fmt[is.na(ordem_faixa), ordem_faixa := 999L]
  out_fmt[, ordem_politica := match(politica, catalogo$marcador_label)]
  out_fmt[is.na(ordem_politica), ordem_politica := 999L]
  data.table::setorder(out_fmt, ordem_faixa, ordem_politica)

  out[, c("ordem_faixa", "ordem_politica") := NULL]
  out_fmt[, c("ordem_faixa", "ordem_politica") := NULL]

  list(raw = out, formatted = out_fmt)
}

tabelas_diferenca_policy_phc <- criar_tabela_diferenca_policy_phc(resultados_v10)

tabela_diferenca_policy_phc_raw <- tabelas_diferenca_policy_phc$raw
tabela_diferenca_policy_phc_formatada <- tabelas_diferenca_policy_phc$formatted

# Saidas em formato de base de dados e tabela legivel.
# Os arquivos *_ptbr.csv usam ponto e virgula como separador e virgula decimal.
fwrite_safe(
  tabela_diferenca_policy_phc_raw,
  file.path(dir_tabelas, "05_policy_phc_difference_raw.csv")
)

fwrite_ptbr_safe(
  tabela_diferenca_policy_phc_raw,
  file.path(dir_tabelas, "05_policy_phc_difference_raw_ptbr.csv")
)

fwrite_ptbr_safe(
  tabela_diferenca_policy_phc_formatada,
  file.path(dir_tabelas, "06_policy_phc_difference_table_ptbr.csv")
)

# Excel dedicado, com uma aba numerica e outra pronta para leitura.
wb_diff <- openxlsx::createWorkbook()

add_sheet_safe(wb_diff, "readable_table", tabela_diferenca_policy_phc_formatada)
add_sheet_safe(wb_diff, "raw_numeric", tabela_diferenca_policy_phc_raw)

if (length(openxlsx::sheets(wb_diff)) > 0) {
  ...
  for (sh in openxlsx::sheets(wb_diff)) {
    openxlsx::addStyle(
      wb_diff,
      sheet = sh,
      style = header_style,
      rows = 1,
      cols = 1:max(1, ncol(if (sh == "readable_table") tabela_diferenca_policy_phc_formatada else tabela_diferenca_policy_phc_raw)),
      gridExpand = TRUE
    )
    openxlsx::freezePane(wb_diff, sheet = sh, firstRow = TRUE)
    openxlsx::setColWidths(wb_diff, sheet = sh, cols = 1:60, widths = "auto")
  }
}

openxlsx::saveWorkbook(
  wb_diff,
  file.path(dir_tabelas, "policy_phc_difference_table.xlsx"),
  overwrite = TRUE
)
...
add_sheet_safe(wb, "phc_difference_raw", tabela_diferenca_policy_phc_raw)
add_sheet_safe(wb, "phc_difference_table", tabela_diferenca_policy_phc_formatada)
...
readme <- c(
  ...
  "The script now exports a dedicated policy-only versus policy-plus-PHC difference table.",
  "",
  "Interpretation:",
  ...
  "tables/05_policy_phc_difference_raw.csv: numeric table for analysis.",
  "tables/05_policy_phc_difference_raw_ptbr.csv: semicolon-separated CSV with comma decimals.",
  "tables/06_policy_phc_difference_table_ptbr.csv: readable table for Excel in Brazilian locale.",
  "tables/policy_phc_difference_table.xlsx: formatted Excel workbook.",
  "",
  "Caution:",
...
message("Policy + PHC difference table: ", file.path(dir_tabelas, "policy_phc_difference_table.xlsx"))
message("CSV PT-BR table: ", file.path(dir_tabelas, "06_policy_phc_difference_table_ptbr.csv"))
